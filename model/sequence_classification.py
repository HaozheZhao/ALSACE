...
class BertPrefixForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        prefix_attention_mask = torch.ones(batch_size, self.model_args.pre_seq_len).to(self.bert.device)
...
class BertPrefixForSequenceClassification_backup(BertPreTrainedModel):
    def __init__(self, config, model_args):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
