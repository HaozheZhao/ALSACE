...
class GeneralTaskHelper(TaskHelper):
    ...
    def logits2pred(self, inputs, logits):
        ...
        elif self.config.model_type == "xglm":
            # cls_index = torch.where((inputs['input_ids'] == self.tokenizer.cls_token_id))[1]
            # cls_index = -3
            logits = logits[:,-1,:]
            pred_idx = inputs["label_token_ids_list"][0,:].view(-1)
            y_pred_result = logits[:,pred_idx]
...
class MlamaTaskHelper(TaskHelper):
    def __init__(self, model_args, model, tokenizer):
        """
        Create a new task helper.
        :param wrapper: The wrapper for the language model being used.
    ...
    def logits2pred(self, inputs, logits):
        ...
        if self.config.model_type in ["t5","mt5"]:
            ...
            num_of_token=len(inputs["labels"][0])
            
            # label_mask = (input_ids == self.model_args.tokenizer.mask_token_id).nonzero().reshape(batch_size, num_mask_token, -1)[:, :, -1].to(self.bert.device) # batch_size * num_mask
            # label_mask = torch.nonzero(inputs["labels"] != -100).reshape(batch_size, num_of_token, -1)[:, :, -1].to(self.model.device) # batch_size * num_mask
            preds=[]
            max_length = 5
            for idx,la in enumerate(inputs["labels"]):
                la_mask = torch.where(la!=-100)[0][1:]
                lo = logits[idx,la_mask].argmax(1)
                pad_length = max_length-len(lo)
                lo = pad(lo,(0,pad_length))
                preds.append(lo)
...
TASK_HELPERS = {
    ...
    "wsc": WscTaskHelper,
    "mlama": MlamaTaskHelper,
    "geolama": MlamaTaskHelper,
}